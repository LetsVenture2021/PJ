...
20 functions derived from PJ's operating instructions:
  ...
  Digital property (§18).......... fetch_url, check_website
...
def create_calendar_event(title: str, start_iso: str, duration_minutes: int = 60,
    ...
    cal_clause = (f'set targetCal to calendar "{q(calendar_name)}"'
                  if calendar_name else "set targetCal to first calendar whose writable is true")
    script = f'''
    set startD to current date
    ...
    if "error" in r:
        return r
    return {"status": "created", "title": title, "start": start.isoformat(),
            "duration_minutes": dur, "calendar": r["stdout"]}
...
def list_reminders(list_name: str = "") -> dict:
    ...
    if "error" in r:
        return r
    items = [dict(zip(("title", "due"),
                      (p.strip() for p in line.split("|", 1))))
             for line in r["stdout"].splitlines() if line.strip()]
    return {"list": list_name or "default", "count": len(items),
            "reminders": items[:50]}
...
def create_reminder(title: str, due_iso: str = "", list_name: str = "") -> dict:
    ...
    if due_iso:
        ...
        due_clause = (f'set dueD to current date\n'
    ...
    if "error" in r:
        return r
    return {"status": "created", "title": title, "due": due_iso or None,
...
def draft_email(to: str, subject: str, body: str) -> dict:
    ...
    if "error" in r:
        return r
    return {"status": "draft_open_in_mail", "to": to, "subject": subject,
...
def list_recent_emails(count: int = 10) -> dict:
    ...
    if "error" in r:
        return r
    msgs = [dict(zip(("received", "sender", "subject"),
                     (p.strip() for p in line.split("|", 2))))
            for line in r["stdout"].splitlines() if "|" in line]
...
def log_contact_interaction(person: str, summary: str, org: str = "",
                            channel: str = "", follow_up: str = "") -> dict:
...
def search_contact_history(person: str) -> dict:
    ...
    with _db() as conn:
        rows = conn.execute(
            ...
            "ORDER BY created_at DESC LIMIT 25", (like, like)).fetchall()
    return {"count": len(rows), "interactions": [
        dict(zip(("person", "org", "channel", "summary", "follow_up",
                  "created_at"), r)) for r in rows]}


# --------------------------------------------- projects & portfolio (§12)
def create_project(name: str, org: str = "", next_milestone: str = "",
                   milestone_due: str = "", notes: str = "") -> dict:
...
def update_project(project_id: str, status: str = "", next_milestone: str = "",
                   milestone_due: str = "", notes: str = "") -> dict:
    """Update a project's status, milestone, or notes."""
    sets, vals = [], []
    for col, val in (("status", status), ("next_milestone", next_milestone),
                     ("milestone_due", milestone_due), ("notes", notes)):
        if val:
            sets.append(f"{col}=?")
            vals.append(val)
    if not sets:
    ...
    with _db() as conn:
        cur = conn.execute(
            f"UPDATE co_projects SET {', '.join(sets)} WHERE id=?", vals)
    return {"project_id": project_id,
            "status": "updated" if cur.rowcount else "not_found"}
...
def portfolio_review() -> dict:
    ...
    with _db() as conn:
        rows = conn.execute(
            ...
            "WHERE status != 'archived' ORDER BY org, name").fetchall()
    projects, flags = [], []
    for r in rows:
        p = dict(zip(("id", "name", "org", "status", "next_milestone",
                      "milestone_due", "notes", "updated_at"), r))
        try:
            ...
            if p["status"] == "active" and p["days_since_update"] > 14:
                flags.append(f"{p['name']}: no update in "
                             f"{p['days_since_update']}d")
        except ValueError:
            pass
        if p["milestone_due"]:
            try:
                if datetime.fromisoformat(p["milestone_due"]).replace(
                        tzinfo=timezone.utc) < now:
                    flags.append(f"{p['name']}: milestone "
                                 f"'{p['next_milestone']}' overdue")
            except ValueError:
                pass
        projects.append(p)
    return {"count": len(projects), "attention_flags": flags,
            "projects": projects}


# --------------------------------------------------- commitments (§13)
def log_commitment(who: str, what: str, due: str = "",
...
def list_commitments(direction: str = "all") -> dict:
    ...
    items = []
    for r in rows:
        c = dict(zip(("id", "who", "what", "due", "direction", "project_id",
                      "created_at"), r))
        if c["due"]:
            try:
                c["overdue"] = datetime.fromisoformat(c["due"]).replace(
                    tzinfo=timezone.utc) < now
            except ValueError:
                c["overdue"] = False
        items.append(c)
...
def complete_commitment(commitment_id: str) -> dict:
    ...
    with _db() as conn:
        cur = conn.execute(
            "UPDATE co_commitments SET status='done' WHERE id=?",
            (commitment_id,))
...
def log_opportunity(name: str, org: str = "", stage: str = "lead",
                    value_usd: float = 0, probability: float = 0.3,
                    next_step: str = "", notes: str = "") -> dict:
    ...
    with _db() as conn:
        conn.execute(
            ...
            (oid, name, org, stage, float(value_usd),
             min(max(float(probability), 0), 1), next_step, notes))
    return {"status": "logged", "opportunity_id": oid, "name": name}


def update_opportunity(opportunity_id: str, stage: str = "",
                       value_usd: float = -1, probability: float = -1,
                       next_step: str = "", status: str = "",
                       notes: str = "") -> dict:
    ...
    if value_usd >= 0:
        sets.append("value_usd=?"); vals.append(float(value_usd))
    if probability >= 0:
        sets.append("probability=?")
        vals.append(min(max(float(probability), 0), 1))
    if next_step:
        sets.append("next_step=?"); vals.append(next_step)
    if status:
    ...
    with _db() as conn:
        cur = conn.execute(
            f"UPDATE co_opportunities SET {', '.join(sets)} WHERE id=?", vals)
...
def pipeline_review() -> dict:
    ...
    with _db() as conn:
        rows = conn.execute(
            ...
            "ORDER BY value_usd DESC").fetchall()
    opps, stages, stalled = [], {}, []
    total = weighted = 0.0
    for r in rows:
        o = dict(zip(("id", "name", "org", "stage", "value_usd",
                      "probability", "next_step", "updated_at"), r))
        total += o["value_usd"]
        ...
        try:
            ...
            upd = upd if upd.tzinfo else upd.replace(tzinfo=timezone.utc)
            if (now - upd).days > 14:
                stalled.append(f"{o['name']} ({o['stage']}, "
                               f"{(now - upd).days}d idle)")
        except ValueError:
            pass
        opps.append(o)
    return {"open_count": len(opps), "total_value_usd": round(total, 2),
            "weighted_value_usd": round(weighted, 2), "by_stage": stages,
            "stalled": stalled, "opportunities": opps[:25]}


# ------------------------------------------------ decision memory (§20)
def log_decision(decision: str, rationale: str = "",
                 alternatives: str = "", context: str = "") -> dict:
...
def search_decisions(query: str) -> dict:
    ...
    return {"count": len(rows), "decisions": [
        dict(zip(("id", "decision", "rationale", "alternatives", "context",
                  "created_at"), r)) for r in rows]}


# ------------------------------------------------ risk register (§21)
def log_risk(risk: str, severity: str = "medium", mitigation: str = "",
             project_id: str = "") -> dict:
    ...
    with _db() as conn:
        conn.execute(
            "INSERT INTO co_risks (id, risk, severity, mitigation, "
            "project_id) VALUES (?,?,?,?,?)",
...
def daily_brief() -> dict:
    ...
    with _db() as conn:
        risks = conn.execute(
            ...
            "WHEN 'medium' THEN 2 ELSE 3 END LIMIT 10").fetchall()
    ...
    return {
        ...
        "overdue_commitments": [c for c in commitments["commitments"]
                                if c.get("overdue")][:5],
        "pipeline": {k: pipeline[k] for k in
                     ("open_count", "total_value_usd", "weighted_value_usd",
                      "stalled")},
        "portfolio_flags": portfolio["attention_flags"],
        "open_risks": [{"risk": r[0], "severity": r[1], "mitigation": r[2]}
                       for r in risks],
        "today_calendar": (calendar.get("events", [])
                           if "error" not in calendar else
                           f"calendar unavailable: {calendar['error']}"),
    }
...
CHIEFOPS_SCHEMAS = [
    ...
    _fn("create_reminder",
        ...
        {"title": _S, "due_iso": {**_S, "description": "e.g. 2026-07-28T09:00"},
         "list_name": _S}, ["title"]),
    _fn("draft_email",
    ...
    _fn("list_recent_emails",
        "List the newest messages in the Apple Mail inbox (sender, subject, date).",
        {"count": {**_I, "description": "1-25, default 10"}}),
    _fn("log_contact_interaction",
        "Record an interaction with a person (call, meeting, email) for relationship continuity.",
        {"person": _S, "summary": _S, "org": _S, "channel": _S,
         "follow_up": _S}, ["person", "summary"]),
    _fn("search_contact_history",
    ...
    _fn("create_project",
        ...
        {"name": _S, "org": {**_S, "description": "e.g. Aimhi Deal Desk"},
         "next_milestone": _S,
         "milestone_due": {**_S, "description": "ISO date"},
         "notes": _S}, ["name"]),
    _fn("update_project",
        "Update a project's status (active/paused/done/archived), milestone, or notes.",
        {"project_id": _S, "status": _S, "next_milestone": _S,
         "milestone_due": _S, "notes": _S}, ["project_id"]),
    _fn("portfolio_review",
        "Review all projects with stale-update and overdue-milestone flags."),
    _fn("log_commitment",
        "Track a commitment: something the user owes (owed_by_me) or is owed (owed_to_me).",
        {"who": {**_S, "description": "Counterparty"},
         "what": _S, "due": {**_S, "description": "ISO date"},
         "direction": {**_S, "enum": ["owed_by_me", "owed_to_me"]},
         "project_id": _S}, ["who", "what"]),
    _fn("list_commitments",
        "List open commitments with overdue flags.",
        {"direction": {**_S, "enum": ["owed_by_me", "owed_to_me", "all"]}}),
    _fn("complete_commitment",
        "Close a commitment as fulfilled.",
        {"commitment_id": _S}, ["commitment_id"]),
    _fn("log_opportunity",
        "Add a revenue opportunity to the pipeline.",
        {"name": _S, "org": _S,
         "stage": {**_S, "enum": ["lead", "qualified", "proposal",
                                  "negotiation", "closing"]},
         "value_usd": _N, "probability": {**_N, "description": "0-1"},
         "next_step": _S, "notes": _S}, ["name"]),
    _fn("update_opportunity",
        "Update an opportunity's stage, value, probability, next step, or mark won/lost.",
        {"opportunity_id": _S, "stage": _S, "value_usd": _N,
         "probability": _N, "next_step": _S,
         "status": {**_S, "enum": ["open", "won", "lost"]},
         "notes": _S}, ["opportunity_id"]),
    _fn("pipeline_review",
