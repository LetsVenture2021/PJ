...
_SEED_TEMPLATES = {
    ...
    "meeting_memo": {
        ...
        "sections": ["Attendees", "Context", "Discussion", "Decisions", "Action Items"],
    ...
    "status_report": {
        ...
        "sections": ["Period", "Highlights", "Metrics", "Blockers", "Next Period Plan"],
...
def _upsert_template(
    conn, name: str, description: str, required_sections: list, optional_sections: list
) -> dict:
    row = conn.execute(
...
def create_doc_template(
    name: str, description: str, sections_json: str, optional_sections_json: str = "[]"
) -> dict:
    ...
    try:
        ...
        return {
            "error": "sections_json must be a non-empty JSON array of "
    ...
    with _db() as conn:
        ...
        _register_template_alias(conn, name, name, canonical_title=name, overwrite_existing=True)
    return {
        "status": "saved",
        "template": name,
...
def list_doc_templates(query: str = "", limit: int = 50, summary_only: bool = False) -> dict:
    """List or summarize templates without flooding tool context."""
    query = str(query or "").strip()
    limit = max(1, min(int(limit or 50), 100))
    like = f"%{query}%"
    with _db() as conn:
        total = conn.execute("SELECT COUNT(*) FROM docops_templates").fetchone()[0]
        matched = conn.execute(
            "SELECT COUNT(*) FROM docops_templates WHERE name LIKE ? OR description LIKE ?",
            (like, like),
        ).fetchone()[0]
        if summary_only:
            return {
...
def docops_inventory_summary() -> dict:
    ...
    with _db() as conn:
        template_count = conn.execute("SELECT COUNT(*) FROM docops_templates").fetchone()[0]
        alias_count = conn.execute("SELECT COUNT(*) FROM docops_template_aliases").fetchone()[0]
...
def import_doc_templates_from_knowledge_pack_text(
    ...
    if mismatched_markers:
        ...
        for start_marker, end_marker in mismatched_markers[:10]:
            framing_errors.append(
                f"ITEM marker IDs do not match (start={start_marker!r}, end={end_marker!r})"
            )
    ...
    with _db() as conn:
        for idx, extracted in enumerate(blocks, start=1):
            ...
            item_id = str(
                _item_value(item, "item_id", "id", "spec_id", "template_id", "item") or marker_id
            ).strip()
            title = str(
                _item_value(item, "canonical_title", "title", "template_title", "name")
            ).strip()
            explicit_template = str(_item_value(item, "template_name", "template", "slug")).strip()
            description = str(_item_value(item, "description", "summary", "purpose")).strip()
            ...
            optional = _as_list(
                _item_value(item, "optional_sections", "optional", "optional_section_names")
            )
            if not required:
                required = _dedupe_nonempty(
                    [m.group(1).strip() for m in re.finditer(r"^##\s+(.+?)\s*$", block, re.M)]
                )
            optional = [s for s in _dedupe_nonempty(optional) if s not in set(required)]
            lifecycle_values = [
            ...
            provisional = _is_truthy(_item_value(item, "provisional", "is_provisional")) or any(
                value
                in {
                    "provisional",
                    "provisional_instructional_spec",
                    "draft",
                    "experimental",
                    "wip",
                }
            ...
            if not required:
                skipped_invalid += 1
                errors.append(
                    f"item {idx}: missing required sections (title={title!r}, item_id={item_id!r})"
                )
                continue
            ...
            if not re.fullmatch(r"[a-z][a-z0-9_]*", template_name):
                skipped_invalid += 1
                errors.append(
                    f"item {idx}: could not derive valid template name from {template_seed!r}"
                )
                continue
...
def _validate_sections(template_row, sections: dict) -> list:
    ...
    for sec in sections:
        if sec not in known:
            errors.append(f"unknown section '{sec}' (template defines: {sorted(known)})")
    return errors
...
def _render(
    ...
    lines = [
        ...
        f"**Status:** DRAFT · **Created:** {_now()}" + (f" · **Tags:** {tags}" if tags else ""),
        "",
    ]
...
def _write_version(
    conn, doc_id, version, title, template, tpl_version, sections, tags, change_note
):
    tpl_row = conn.execute(
        "SELECT name, version, description, sections, optional_sections "
        "FROM docops_templates WHERE name=?",
        (template,),
    ).fetchone()
    ordered = json.loads(tpl_row[3]) + json.loads(tpl_row[4])
    content = _render(title, template, doc_id, version, ordered, sections, tags)
    ...
    return {
        ...
        "next": (
            "resolve markers then finalize_document"
            if markers
            else "finalize_document when approved"
        ),
    }


def draft_document(
    template: str, title: str, sections_json: str, tags: str = "", finalize: bool = False
) -> dict:
    ...
    try:
        ...
        return {"error": "sections_json must be a JSON object {section_name: markdown_body}"}
    template_ref = str(template or "").strip()
    with _db() as conn:
        resolved_template, resolution_source = _resolve_template_name(conn, template_ref)
        if not resolved_template:
            names = [r[0] for r in conn.execute("SELECT name FROM docops_templates").fetchall()]
            ...
            return {
                ...
                "available": names,
                "recent_aliases": [{"alias": r[0], "template": r[1]} for r in alias_hits],
            }
        tpl = conn.execute(
            "SELECT name, version, description, sections, optional_sections "
            "FROM docops_templates WHERE name=?",
        ...
        result = _write_version(
            conn, doc_id, 1, title, resolved_template, tpl[1], sections, tags, "initial draft"
        )
    ...
    if finalize and not result.get("unresolved_markers"):
        ...
        result["next"] = (
            "finalize was requested but unresolved markers "
            "remain; resolve via revise_document first"
        )
    return _attach_source_artifact(result)


def revise_document(doc_id: str, sections_json: str, change_note: str = "") -> dict:
    ...
    with _db() as conn:
        ...
        if errors:
            return {"status": "rejected", "errors": errors}
        conn.execute(
            "UPDATE docops_documents SET status='superseded' WHERE doc_id=? AND version=?",
            (doc_id, version),
        )
        result = _write_version(
            conn,
...
def _presentation_spec(
    ...
    raw["subtitle"] = str(
        subtitle or raw.get("subtitle") or fallback_spec.get("subtitle") or ""
    ).strip()
...
def _presentation_companion_matches(content: str, spec: dict) -> bool:
    parts = content.split("\n\n", 2)
    return len(parts) == 3 and parts[2].strip() == _presentation_markdown_body(spec).strip()


def _write_presentation_version(
    conn, doc_id: str, version: int, title: str, spec: dict, tags: str, change_note: str
) -> dict:
    template = "slide_presentation"
    tpl = conn.execute("SELECT version FROM docops_templates WHERE name=?", (template,)).fetchone()
    if not tpl:
...
def revise_presentation(
    ...
    with _db() as conn:
        ...
        prior_row = conn.execute(
            "SELECT spec_json FROM docops_presentation_specs WHERE doc_id=? AND version=?",
            (doc_id, version),
        ).fetchone()
...
def finalize_document(doc_id: str) -> dict:
    ...
    with _db() as conn:
        ...
        if spec_row:
            ...
            try:
                normalized_spec = presentationops.normalize_spec(json.loads(spec_row[0]))
            except presentationops.PresentationValidationError as exc:
                return {
                    "status": "blocked",
            ...
            if not _presentation_companion_matches(content, normalized_spec):
                return {
                    "status": "blocked",
                    "reason": (
                        "presentation companion diverges from the authoritative slide specification"
                    ),
                }
...
def _export_sections(source: str) -> list[tuple[str, str]]:
    ...
    sections.extend((match.group(1).strip(), match.group(2).strip()) for match in matches)
    return sections
...
def _plain_markdown(value: str) -> str:
    ...
    lines = [re.sub(r"[ \t]+", " ", _html.unescape(line)).strip() for line in rendered.splitlines()]
...
def _write_pdf_export(out_path: Path, title: str, sections, metadata: dict) -> None:
    ...
    def render(path):
        ...
        story = [
            ...
            Paragraph(
                f"{metadata['date']} · {'FINAL' if metadata['is_final'] else 'DRAFT'}",
                meta_style,
            ),
        ]
...
def _write_powerpoint_export(out_path: Path, title: str, sections, metadata: dict) -> None:
    ...
    def add_textbox(
        slide, left, top, width, height, text, size, color, *, bold=False, alignment=PP_ALIGN.LEFT
    ):
    ...
    def decorate(slide):
        line = slide.shapes.add_shape(1, Inches(0.55), Inches(0.38), Inches(12.2), Inches(0.04))
        line.fill.solid()
        line.fill.fore_color.rgb = primary
        line.line.fill.background()
        add_textbox(
            slide, Inches(0.55), Inches(0.05), Inches(1.5), Inches(0.3), "AIMHI", 9, ink, bold=True
        )
        add_textbox(
            slide,
            Inches(9.5),
        ...
        add_textbox(
            ...
            Inches(7),
            Inches(0.25),
            f"{metadata['doc_id']} v{metadata['version']} · integrity {metadata['sha256'][:12]}",
            7,
            muted,
        )
        if not metadata["is_final"]:
...
def _write_excel_export(out_path: Path, title: str, sections, metadata: dict) -> None:
    ...
    def render(path):
        ...
        for row_index, (heading, markdown_body) in enumerate(sections, start=header_row + 1):
...
def _export_lock(doc_id: str, version: int, format: str):
    ...
    lock_path = lock_dir / (f"{_slug(doc_id)}-{int(version)}-{_slug(format)}.lock")
...
def register_external_artifact(
    doc_id: str, version: int, format: str, path: str | Path, *, audience_ready: bool = True
) -> dict:
...
def tombstone_export_artifact(
    artifact_id: str, *, connection: sqlite3.Connection | None = None
) -> bool:
    ...
    try:
        with _export_lock(doc_id, version, "md"):
            ...
            if not expected_sha or _hash(source.read_text()) != expected_sha:
                raise ValueError("source document does not match its governed SHA-256")
            staging = EXPORTS_DIR / "sources" / source.name
            _atomic_copy(source, staging)
            if _hash(staging.read_text()) != expected_sha:
                raise ValueError("source artifact copy failed governed-hash validation")
...
def list_export_artifacts(doc_id: str = "", version: int = 0, format: str = "") -> dict:
...
def export_document(doc_id: str, format: str = "html", version: int = 0) -> dict:
    ...
    if format not in supported_formats:
        return {"error": ("format must be one of: " + ", ".join(supported_formats))}
    ...
    if format == "md":
        ...
        return {
            ...
            "artifact": artifact,
            "note": (
                "governed final Markdown source" if is_final else "governed draft Markdown source"
            ),
        }
    if format == "pptx":
        spec_row = next((row for row in presentation_rows if row[0] == ver), None)
        if not spec_row:
            return {
                "status": "rejected",
                "error": (
    ...
    body_html = _markdown.markdown(body_md, extensions=["tables", "fenced_code"])
    date = datetime.now(timezone.utc).strftime("%B %d, %Y")
    watermark = "" if is_final else '<div class="watermark">DRAFT</div>'
    meta = f'<p class="docmeta">{date}</p>'
    footer = (
        f"<footer><span>{_html.escape(title)} · {doc_id} v{ver} · "
    ...
    return {
        ...
        "note": (
            "clean final export"
            if is_final
            else "non-final version exported with DRAFT watermark; "
...
DOCOPS_SCHEMAS = [
    {
        ...
        "description": (
            "Search or summarize DocOps templates. Use "
    ...
    },
    {
        ...
        "description": (
            "Create or upgrade a versioned document template that "
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "name": {"type": "string", "description": "lowercase_identifier template name"},
    ...
    },
    {
        ...
        "name": "draft_document",
        "description": (
            "Create a mission-critical document from a template. "
    ...
    },
    {
        ...
        "description": (
            "Issue a new version of a document. Pass only the "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "change_note": {"type": "string", "description": "What changed and why"},
            },
    ...
    },
    {
        ...
        "description": (
            "Create a reusable native-presentation source from a "
    ...
    },
    {
        ...
        "description": (
            "Render and register the exact requested deliverable. "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "format": {
                    "type": "string",
                    "enum": ["md", "html", "pdf", "docx", "rtf", "pptx", "xlsx"],
    ...
    },
    {
        ...
        "description": (
            "List registered, integrity-verified document artifacts "
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "status": {"type": "string", "enum": ["draft", "final", "superseded", "all"]},
                "query": {"type": "string", "description": "Keyword in title/tags/doc_id"},
            },
            "required": [],
        },
    },
...
_export_document_core = export_document


def export_document(doc_id: str, format: str = "html", version: int = 0) -> dict:  # noqa: F811
